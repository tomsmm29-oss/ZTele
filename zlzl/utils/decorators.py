...
# استيراد CMD_HELP ضروري عشان المساعدة تشتغل
from zlzl import CMD_HELP, LOAD_PLUG, CMD_LIST, SUDO_LIST
...
def admin_cmd(pattern=None, command=None, **args):
    ...
    if pattern is not None:
        try:
            ...
            # --- تسجيل الأوامر في القوائم (مهم للمساعدة) ---
            if file_test not in CMD_LIST:
                CMD_LIST[file_test] = []
            CMD_LIST[file_test].append(cmd)
            
            # --- تسجيل في CMD_HELP (هذا اللي كان ناقص!) ---
            if file_test not in CMD_HELP:
                CMD_HELP[file_test] = {
                    "info": "لا يوجد وصف",
                    "commands": []
                }
            CMD_HELP[file_test]["commands"].append(cmd)
            # ------------------------------------------------
...
def errors_handler(func):
    async def wrapper(check):
        try:
            ...
            # إصلاح الخطأ القاتل وتعريف check
