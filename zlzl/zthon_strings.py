...
لـ تحليـل اخـر 500 رسالـة بالـجروب
 ...
 𓆩 [𝗭𝗧𝗵𝗼𝗻 𝗨𝘀𝗲𝗿𝗯𝗼𝘁](t.me/zed_thon) 𓆪
    """,
    # ㉖ الردود والترحيب (المحرك الذكي)
    "m26": """
ᯓ [𝗭𝗧𝗵𝗼𝗻 𝗨𝘀𝗲𝗿𝗯𝗼𝘁](t.me/zed_thon) - اوامــر الـردود والـترحـيـب 💬 .
⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆
⪼ `.اضف رد` | `.اضف رد عام` | `.اضف رد خاص`
لـ إضـافـة رد ذكـي (نـص أو ميـديـا) فـي الكـروب، أو عـام، أو بالخـاص.

⪼ `.اضف ترحيب` | `.اضف ترحيب عام` | `.اضف ترحيب خاص`
لـ إضـافـة ترحيـب متحـرك بـأسلـوب زدثـون (اكتـب الكلمـة أو الاسـم بعـد الأمـر).

⪼ `.حذف رد` | `.حذف رد عام` | `.حذف رد خاص`
لـ حـذف رد معيـن (اكتـب كلمـة الـرد بعـد الأمـر أو سيطلبـه منـك).

⪼ `.حذف ترحيب` | `.حذف ترحيب عام` | `.حذف ترحيب خاص`
لـ حـذف ترحيـب معيـن تـم حفـظـه مسـبقـاً.

⪼ `.عرض الردود` | `.عرض الترحيب`
لـ عـرض الـردود أو الترحيبـات المحفوظـة (ويمكـن إضافـة عـام/خـاص للأمـر).

⪼ `.تفعيل اضافه رد` | `.تعطيل اضافه رد`
لـ السمـاح أو (المنـع) للأعضـاء بإضافـة ردودهـم الخاصـة بالـكـروب.

💡 **إضـافـات الـنـص (عنـد إضافـة رد او ترحيـب):**
▹ `#الاسم` : اسـم العضـو
▹ `#يوزره` : منشـن للعضـو
▹ `#اليوزر` : يـوزر المرسـل
▹ `#الايدي` : آيـدي المرسـل
