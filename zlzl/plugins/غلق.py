...
async def lock_zed(event):
    ...
    if gvarstatus(gvar_key):
        return await edit_or_reply(event, f"**•❐• عـذراً .. {lock_name} مـغـلقـة بـالـفـعـل**")
    
    addgvar(gvar_key, "true")
    await edit_or_reply(event, f"**•❐• تـم غـلـق {lock_name} بـنجـاح فـي هـذه الـدردشـة**")
...
async def unlock_zed(event):
    ...
    if not gvarstatus(gvar_key):
        return await edit_or_reply(event, f"**•❐• عـذراً .. {lock_name} غـيـر مـقـفـولـة بـالـفـعـل**")
    
    delgvar(gvar_key)
    await edit_or_reply(event, f"**•❐• تـم الـغـاء قـفـل {lock_name} بـنجـاح فـي هـذه الـدردشـة**")
