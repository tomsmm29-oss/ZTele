...
async def react_normal_raw(client, peer, msg_id, emoji_str):
    ...
    try:
        await client(SendReactionRequest(peer=peer, msg_id=msg_id, reaction=[ReactionEmoji(emoticon=emoji_str)]))
...
async def react_premium_raw(client, peer, msg_id, document_id):
    ...
    try:
        await client(SendReactionRequest(peer=peer, msg_id=msg_id, reaction=[ReactionCustomEmoji(document_id=document_id)]))
    except FloodWaitError as e:
        await asyncio.sleep(e.seconds)
...
async def process_group_reactions(event, peer, emoji_obj, is_premium, count, is_all, target_ids, is_exclude):
    messages_to_react = []
...
async def start_reactions(event):
    ...
    emoji_obj, is_premium, count, is_all, mentioned_users, is_exclude = parse_reaction_args(cmd_text, event.message.entities)
    ...
    if is_future_only:
        ...
        await zed.edit(f"**•❐• تـم تـفـعـيـل الـمـراقـبـة لـلـجـديـد ⩥ {count} رسـالـة**")
        
    else:
        # النظام الفردي (.جر): التفاعل على القديم أولاً
        if is_group:
            done_count = await process_group_reactions(event, peer, emoji_obj, is_premium, count, is_all, target_ids, is_exclude)
        else:
            done_count = await process_pm_reactions(event, peer, emoji_obj, is_premium, count, is_all)
        ...
        if done_count < count and not is_all:
            ...
            await zed.edit(f"**•❐• تـم الـتـفـاعـل ⩥ {done_count}**\n**• يـتـم الانـتـظـار لـلـجـديـد ...**")
        else:
...
async def real_time_reactions(event):
    ...
    if job["current"] % 5 == 0 or job["current"] >= job["target"]:
        try:
            await job["msg_zed"].edit(f"**•❐• تـم الـتـفـاعـل ⩥ {job['current']} رسـالـة**")
        except Exception:
            pass
...
async def cancel_reactions(event):
    ...
    tasks = [remove_reaction_raw(event.client, peer, msg_id) for msg_id in messages_to_clear]
    await asyncio.gather(*tasks)
