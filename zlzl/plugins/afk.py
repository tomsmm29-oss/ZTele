...
# 1. تعديل الاستيراد ليناسب مشروعك (zlzl)
from . import zedub 
...
# 2. حماية الاستيراد عشان ما يكرش لو المتغير ناقص
try:
...
class AFK:
    def __init__(self):
        ...
        # 3. المصيبة الكبرى كانت هنا: {} تسبب كراش عند الطرح، خليناها None
        self.afk_star = None 
        self.afk_end = None
        self.reason = None
        self.msg_link = False
...
async def on_afk(event):
    ...
    if AFK_.USERAFK_ON and not (await event.get_sender()).bot:
        ...
        if AFK_.afk_type == "media":
            # التحقق من وجود الميديا قبل الإرسال
            if AFK_.reason:
