...
async def show_real_users(event):
    zed = await edit_or_reply(
        event, "**•❐• جـاري جـلـب قـائمـة الـحسـابـات الـحـقـيقيـة مـن الـخـاص ..**"
    )
    ...
    if count == 0:
        return await zed.edit(
            "**•❐• لـم يـتـم الـعـثور عـلى أي مـحادثـات خـاصـة مـع أشـخـاص حـقـيقيـيـن**"
        )
...
async def delete_real_users(event):
    zed = await edit_or_reply(
        event,
        "**•❐• جـاري حـذف شـات كـافـة الأشـخـاص مـن الـخـاص ..**\n**• قـد يـستـغرق الأمـر بـعض الـوقـت ..**",
    )
