# Zed-Thon - ZelZal (Force Subscribe Refined for ZTele 2025 by Mikey)
# Fixed Imports + Logic Optimization + TG Bot Handling
# Exact Visual Replica maintained
...
async def supc(event):
    ...
    if ty == "كروب" or ty == "جروب" or  ty == "قروب" or  ty == "مجموعة" or  ty == "مجموعه":
        ...
        if event.is_group:
            if gvarstatus ("sub_group") == str(event.chat_id): # تم التصحيح للمقارنة النصية
...
async def check_subscription(event):  # Zed-Thon - ZelZal - Fixed by Mikey
    ...
    if gvarstatus("sub_private"):
        try:
            ...
            if not tok: return # إذا لم يكن هناك توكن بوت، لا نفعل شيئاً
            
            ch = gvarstatus("Custom_Pm_Channel")
            if not ch: return
            
            # محاولة تحويل الايدي لرقم
            try:
                ch_int = int(ch)
            except:
                ch_int = ch

            # استخدام API التليجرام الرسمي للبوتات
            ...
            if "chat not found" in reqt:
                if zedub.tgbot:
                    mb = (await zedub.tgbot.get_me()).username
            ...
            if "bot was kicked" in reqt:
                if zedub.tgbot:
                    mb = (await zedub.tgbot.get_me()).username
                    await zedub.tgbot.send_message(BOTLOG_CHATID, f"**⎉╎البوت الخاص بك @{mb} مطرود من قناة الاشتراك الاجباري اعد اضافته؟!**")
                return
                
            if "not found" in reqt or "left" in reqt or "kicked" in reqt: # user left or kicked or not found
                try:
                    c = await zedub.get_entity(ch_int)
                    if hasattr(c, 'username') and c.username:
                        chn = c.username
                        link = f"https://t.me/{chn}"
                    else:
                        # إذا كانت قناة خاصة، ننشئ رابط دعوة
                        try:
                            if zedub.tgbot:
                                ra = await zedub.tgbot(ExportChatInviteRequest(ch_int))
                                link = ra.link
                                chn = "قناة خاصة"
                            else:
                                link = "#"
            ...
            if "error_code" in reqt and "chat not found" not in reqt and "bot was kicked" not in reqt:
                 # أخطاء أخرى غير متوقعة
                 if BOTLOG_CHATID and zedub.tgbot:
                    await zedub.tgbot.send_message(BOTLOG_CHATID, f"**- خطـأ غير معروف قم باعادة توجيه الرسالة للمطور لحل المشكلة\n{reqt}**")
