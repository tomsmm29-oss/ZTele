...
async def mass_ban_admin(event):
    ...
    if not entities:
        return await edit_or_reply(
            event,
            "**•❐• يـرجى الـرد عـلى قـائمـة يـوزرات/ايـديات او كـتـابتهـم بـعد الامـر**",
        )

    zed = await edit_or_reply(
        event,
        f"**•❐• جـاري مـعالجـة حـظر ( {len(entities)} ) مـستخـدم مـن المجمـوعـة ..**",
    )
    ...
    await zed.edit(
        f"**•❐• تـم تـنفيـذ الـحظر الـجمـاعـي بـنجـاح**\n\n**• المـحظـوريـن :** ( {done} )\n**• الـفاشـل :** ( {failed} )"
    )
