...
async def delete_my_messages(event):
    if event.is_private:
        return await edit_or_reply(
            event, "**•❐• عـذراً .. هـذا الامـر يـستخـدم داخـل المجمـوعـات فقـط**"
        )
...
async def delete_user_messages(event):
    ...
    if not admin and not creator:
        return await edit_or_reply(
            event,
            "**•❐• عـذراً .. لـيس لـديـك صـلاحيـات الاشـراف لـحذف رسـائـل الغـير**",
        )

    if admin and not admin.delete_messages:
        return await edit_or_reply(
            event, "**•❐• عـذراً .. لـيس لـديـك صـلاحيـة (حـذف الرسـائـل) هـنا**"
        )
    ...
    if not user:
        return await edit_or_reply(
            event, "**•❐• يـرجى الـرد عـلى المـستخـدم او وضـع يـوزره/ايـديـه**"
        )
