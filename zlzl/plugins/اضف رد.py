...
async def add_reply_cmd(event):
    ...
    try:
        ...
        await event.respond("**↢ أرسـل الكـلـمـة المـفـتـاحـيـة (الـتـي سـيـرد عـلـيـها الـبـوت) الان :\n\n•❐• لـ الالغـاء ارسـل `الغاء`**")
    ...
    for db, db_type in databases_to_check:
        ...
        for trigger_word, data in items:
            ...
            if re.search(trigger_regex, text, re.IGNORECASE):
                ...
                # 1. ترحيب اللقب الخاص بالمطور (صامت ومتحرك وسريع)
                if db_type == "grt":
                    if data.get("text") == "anim_zed":
                        # إرسال رسالة البداية الزخرفية الصامتة بنمط متمدد سريع للغاية (0.3 ثانية)
                        ...
                        # التعديل النهائي المكتمل والمختصر متضمناً يوزرك في منتصف الزخرفة بشكل أنيق
                        final_text = "**⋄━─━─━─━─ ✦ #يوزري ✦ ─━─━─━─━⋄**"
