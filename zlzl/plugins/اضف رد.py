...
async def del_reply_cmd(event):
    ...
    if trigger_word:
        ...
        try:
            await event.respond(
                "**↢ أرسـل كـلـمـة الـرد الـتي تـريـد حـذفـهـا الان :\n\n•❐• لـ الالغـاء ارسـل `الغاء`**"
            )
            trigger_msg = await wait_for_next_message(
                event.client, event.chat_id, user_id, current_future, timeout=60
            )
    ...
    if trigger_word in db:
        ...
        await event.respond(
            f"**•❐• تـم حـذف الـرد `{trigger_word}` بـنـجـاح {scope_text} ✓**"
        )
    else:
        await event.respond(
            f"**•❐• عـذراً، الـرد `{trigger_word}` غـيـر مـوجـود حـالـيـاً !**"
        )
...
async def del_greeting_cmd(event):
    ...
    if nickname:
        ...
        try:
            await event.respond(
                "**↢ أرسـل الـلـقـب الـذي تـريـد حـذفـه الان :\n\n•❐• لـ الالغـاء ارسـل `الغاء`**"
            )
            trigger_msg = await wait_for_next_message(
                event.client, event.chat_id, user_id, current_future, timeout=60
            )
    ...
    if found:
        ...
        await event.respond(
            f"**•❐• تـم حـذف تـرحـيـب الـلـقـب `{nickname}` بـنـجـاح {scope_text} ✓**"
        )
    else:
        await event.respond(
            f"**•❐• عـذراً، لـقـب الـتـرحـيـب `{nickname}` غـيـر مـوجـود !**"
        )
...
async def show_replies_cmd(event):
    ...
    if not db:
        await edit_or_reply(
            event,
            f"**•❐• لا تـوجـد ردود مـحـفـوظـة بـقـسـم [{scope_title}] حـالـيـاً !**",
        )
        return
...
async def show_greetings_cmd(event):
    ...
    if not db:
        await edit_or_reply(
            event,
            f"**•❐• لا تـوجـد تـرحـيـبـات لـقـب مـحـفـوظـة فـي [{scope_title}] حـالـيـاً !**",
        )
        return
    ...
    if "nickname" in db:
        ...
        content = (
            "أنـيـمـيـشـن مـتـحـرك فـخـم ✧"
            if db.get("text") == "anim_zed"
            else db.get("text", "")
        )
