...
async def add_greeting_cmd(event):
    ...
    try:
        await event.respond(
            "**↢ أرسـل لـقـبـك الآن (الكلمة المفتاحية التي سيكتبها الأعضاء ليرد البوت بـ يوزرك والزخرفة) :\n\n•❐• لـ الالغـاء ارسـل `الغاء`**"
        )
        ...
        await event.respond(
            f"**•❐• تـم حـفـظ رد الـلـقـب `{nickname}` مـع الـزخـرفـة بـنـجـاح {scope_text} ✓**"
        )
    ...
    # 2. إذا كانت في المجموعات والجروبات
        ...
        if local_grt and text == local_grt.get("nickname"):
            reply_text = format_owner_reply_text(
                local_grt.get("text", ""), owner_entity
            )
            await event.reply(reply_text)
            return
        ...
        if global_grt and text == global_grt.get("nickname"):
            reply_text = format_owner_reply_text(
                global_grt.get("text", ""), owner_entity
            )
            await event.reply(reply_text)
            return
