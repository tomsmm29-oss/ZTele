...
async def w_ban_all(event):
    if event.is_private:
        return await edit_or_reply(
            event, "**- عذراً .. هذا الأمر يُستخدم في المجموعات فقط ؟!**"
        )

    zed = await edit_or_reply(
        event,
        "<b>- جـاري بـدء عمليـة التفليـش عبـر البـوتـات 😈...</b>",
        parse_mode="html",
    )
    ...
    try:
        ...
        await zed.edit(
            f"<b>- تـم الانتهـاء مـن إرسـال أوامـر الحظـر لـ {count} عضـو بنجـاح 🚷</b>",
            parse_mode="html",
        )
