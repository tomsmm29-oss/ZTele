...
async def hunt_gifts_loop(event, client, currency, max_price, status_msg):
    ...
    LOGS.info(
        f"🟢 بدء تشغيل قناص الهدايا | العملة: {currency} | السعر الأقصى: {max_price}"
    )
    ...
    while HUNTER_ACTIVE:
        try:
            ...
            if not gifts_list:
                LOGS.warning(
                    "⚠️ لم يتم العثور على أي هدايا في الكتالوج الأساسي (ربما مشكلة في اتصال الحساب)."
                )
            ...
            for base_gift in gifts_list:
                ...
                try:
                    ...
                    resale_items = await client(
                        GetResaleStarGiftsRequest(
                            gift_id=gift_id, sort_by_price=True, offset="", limit=20
                        )
                    )

                    items_list = getattr(resale_items, "gifts", []) or getattr(
                        resale_items, "items", []
                    )
                    ...
                    for item in items_list:
                        ...
                        try:
                            ...
                            # ---------------------------------------------------------
                            crypto_obj = getattr(
                                item, "crypto_amount", None
                            ) or getattr(item, "ton_value", None)
                            star_obj = getattr(item, "price", None) or getattr(
                                item, "stars", None
                            )
                            ...
                            if crypto_obj is not None:
                                ...
                                item_price = (
                                    val_float / 1e9 if val_float > 1e6 else val_float
                                )
                                is_ton = True
                        ...
                        if currency == "تون" and is_ton and 0 < item_price <= max_price:
                            ...
                            currency == "نجوم"
                            and is_stars
                            and 0 < item_price <= max_price
                        ):
                            matched = True

                        # إذا طابقت المواصفات
                        if matched:
                            item_slug = getattr(item, "slug", None) or getattr(
                                item, "id", gift_id
                            )
                            ...
                            if gift_link not in FOUND_GIFTS:
                                ...
                                LOGS.info(
                                    f"🎉 تم اصطياد هدية! السعر: {item_price} {currency} | الرابط: {gift_link}"
                                )
                    ...
                    LOGS.warning(
                        f"⏳ فلود ويت من تليجرام! تم إيقاف الفحص مؤقتاً لمدة {e.seconds} ثانية."
                    )
                    await asyncio.sleep(e.seconds + 1)
                except Exception as req_err:
                    LOGS.error(
                        f"❌ خطأ غير متوقع أثناء طلب تفاصيل الهدية: {req_err}\n{traceback.format_exc()}"
                    )
            ...
            LOGS.error(
                f"❌ انهيار في الحلقة الرئيسية: {main_err}\n{traceback.format_exc()}"
            )
            await asyncio.sleep(5)
...
async def start_hunter(event):
    ...
    if not HAS_API:
        return await edit_or_reply(
            event,
            "**•❐• عـذراً .. مـكتبـة تـليـثون لـديـك قـديمـة ولا تـدعـم دوال الـهـدايـا الـجديـدة ⚠️**",
        )

    if HUNTER_ACTIVE:
        return await edit_or_reply(
            event,
            "**•❐• صـائـد الـهـدايـا يـعمـل بـالـفعـل .. لإيـقـافـه أرسـل ⩥** `.ايقاف مطوره`",
        )
    ...
    try:
        ...
        return await edit_or_reply(
            event, "**•❐• عـذراً .. الـسـعـر الـذي أدخـلـته غـيـر صـالـح ⚠️**"
        )
    ...
    event.client.loop.create_task(
        hunt_gifts_loop(event, client, currency, max_price, status_msg)
    )
...
async def stop_hunter(event):
    ...
    if not HUNTER_ACTIVE:
        return await edit_or_reply(
            event, "**•❐• صـائـد الـهـدايـا مـتـوقـف بـالـفـعـل ⚠️**"
        )
...
async def stop_counter(event):
    ...
    SHOW_COUNTER = False
    await edit_or_reply(
        event, "**•❐• تـم إيـقـاف الـعـداد الـحـي بـنـجـاح (لـتجـنـب الـفـلـود) ✓**"
    )
