...
# 📦 استدعاء النصوص
# =========================
from zlzl.zthon_texts import FOOTER_TEXT, HEADER_TEXT, TITLES, get_full_menu
...
def get_pyro_keyboard(page):
    ...
    for i, icon in enumerate(all_buttons[start:end]):
        ...
        temp_row.append(
            InlineKeyboardButton(f" {icon} ", callback_data=f"m{real_index}|{page}")
        )
        if len(temp_row) == 3:
    ...
    if page > 1:
        ...
        nav_row.append(
            InlineKeyboardButton(
                f"⪻ ❨ {(p_num-1)*12+1} ⇄ {p_num*12} ❩", callback_data=f"page_{p_num}"
            )
        )
    else:
    ...
    if end < len(all_buttons):
        ...
        nav_row.append(
            InlineKeyboardButton(
                f"❨ {(n_num-1)*12+1} ⇄ {next_end} ❩ ⪼", callback_data=f"page_{n_num}"
            )
        )
    else:
    ...
    keyboard.append(
        [InlineKeyboardButton("❎ اغــلاق القائمــة", callback_data="close_all")]
    )
            ...
            results=[
                InlineQueryResultArticle(
                    ...
                    input_message_content=InputTextMessageContent(
                        generate_page_text(name, 1), disable_web_page_preview=True
                    ),
    ...
    try:
        if data == "close_all":
            try:
                ...
                await callback_query.edit_message_text("✅ تم إغلاق القائمة")
            return
                ...
                back_btn = InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "⪼ رجــوع للقائمــة ⪻",
                                callback_data=f"page_{origin_page}",
                            )
                        ]
                    ]
                )
                await callback_query.edit_message_text(
                    SECTION_DETAILS[section_key],
...
async def launch_menu(event):
    if not bot_token:
        return await event.edit("⚠️ خطأ في توكن البوت")
    await event.edit("⌛️")
    try:
        ...
        if results:
            ...
            await event.edit("⚠️ فشل في جلب القائمة.")
    except Exception as e:
...
async def direct_txt(event):
    ...
    if f"m{num}" in SECTION_DETAILS:
        await event.edit(SECTION_DETAILS[f"m{num}"])
