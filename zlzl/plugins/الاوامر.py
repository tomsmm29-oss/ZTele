...
# استيراد أدوات السورس (تأكد أن مسارات الاستيراد صحيحة حسب هيكلية السورس لديك)
...
from . import SUDO_LIST, edit_delete, edit_or_reply, BOTLOG, BOTLOG_CHATID, HEROKU_APP, mention
...
async def inline_handler(event):
    ...
    if query.startswith("الاوامر"):
        result = builder.article(title="zedub", text=CMD_TEXTS["main"], buttons=MAIN_BUTTONS, link_preview=False)
        await event.answer([result] if result else None)
    
    # معلومات زدثون والقنوات
    elif query.startswith("زدثون"):
    ...
    # الأوامر التعليمية (شروحات)
    elif query.startswith("اليدوي") or query.startswith("الفيزا") or query.startswith("الهيروكو") or query.startswith("التنصيب"):
        user = gvarstatus("who_id")
        name_zed = "المستخدم"
        if user:
            try:
                user_zed = await event.client.get_entity(int(user))
                name_zed = user_zed.first_name
            except: pass
        
        url_btn = ""
        text_msg = ""
        if query.startswith("اليدوي"):
            text_msg = f"• عـزيـزي ** {name_zed} \n• اليـك شـرح اعـادة التشغيـل اليـدوي↓**"
            url_btn = "https://t.me/zzzlvv/62"
        elif query.startswith("الفيزا"):
            text_msg = f"• عـزيـزي ** {name_zed} \n• اليـك الفيـزا الخـاصـه بهيـروكـو↓**"
            url_btn = "https://t.me/zzzlvv/64"
        elif query.startswith("الهيروكو"):
            text_msg = f"• عـزيـزي ** {name_zed} \n• اليـك شـرح إنشـاء حسـاب هيـروكـو↓**"
            url_btn = "https://t.me/zzzlvv/63"
        elif query.startswith("التنصيب"):
            text_msg = f"• عـزيـزي ** {name_zed} \n• اليـك شـرح تنصيب السـورس ع هيـروكـو↓**"
            url_btn = "https://t.me/zzzlvv/65"

        buttons = [[Button.url("اضغــط هنــا", url_btn)]]
        result = builder.article(title="zedub", text=text_msg, buttons=buttons, link_preview=False)
        await event.answer([result] if result else None)


# --- معالج الضغط على الأزرار (Callback Handler) الموحد ---
@zedub.tgbot.on(CallbackQuery(data=re.compile(b"zed_(.*)")))
async def unified_callback_handler(event):
    ...
    # التحقق من أن الضغط من صاحب البوت
    if event.query.user_id != zedub.uid:
        return await event.answer("⚠️ هـذه الازرار ليست لك.", alert=True)

    try:
        # زر الرجوع للقائمة الرئيسية
        if data == "main_menu":
            await event.edit(CMD_TEXTS["main"], buttons=MAIN_BUTTONS, link_preview=False)
        
        # أزرار القوائم الفرعية (cmd_1, cmd_2...)
        elif data.startswith("cmd_"):
            page_num = data.split("_")[2] # سيأخذ الرقم بعد zed_cmd_
            if page_num in CMD_TEXTS:
                await event.edit(CMD_TEXTS[page_num], buttons=BACK_BUTTON, link_preview=False)
            else:
                await event.answer("الصفحة غير موجودة", alert=True)

    except Exception as e:
        # تجاهل خطأ عدم تغير الرسالة (عند الضغط مرتين)
        if "MessageNotModified" in str(e):
            return
        LOGS.error(f"Error in callback: {e}")

# --- الأوامر النصية التي تشغل الانلاين ---

@zedub.zed_cmd(pattern="الاوامر(?: |$)(.*)")
async def zed_command_main(event):
    if event.fwd_from: return
    
    bot_username = Config.TG_BOT_USERNAME
    if not bot_username or bot_username == "None":
        try:
            bot_me = await zedub.tgbot.get_me()
            bot_username = bot_me.username
        except:
            return await edit_or_reply(event, "**⚠️ يجب تفعيل البوت المساعد أولاً وتشغيله.**")
            
    if event.reply_to_msg_id:
        await event.get_reply_message()

    try:
        results = await event.client.inline_query(bot_username, "الاوامر")
        if results:
            await results[0].click(event.chat_id)
            await event.delete()
        else:
            await edit_or_reply(event, "**⚠️ لم يتم العثور على نتائج. تأكد من تفعيل Inline Mode في @BotFather للبوت المساعد.**")
    except Exception as e:
        # رسالة احتياطية نصية في حال فشل الانلاين (بسبب صلاحيات المجموعة مثلاً)
        await edit_or_reply(event, f"**عذراً، لا يمكن فتح القائمة الشفافة في هذه المجموعة.**\nالسبب: قد يكون البوت المساعد غير مشرف أو مقيد.\n\nاستخدم `.اوامري` لعرض الأوامر كنص عادي.")

@zedub.zed_cmd(pattern="زدثون")
async def zed_command_zthon(event):
    if event.fwd_from: return
    bot_username = Config.TG_BOT_USERNAME
    try:
        results = await zedub.tgbot.inline_query(bot_username, "زدثون")
        await results[0].click(event.chat_id)
        await event.delete()
    except: pass

@zedub.zed_cmd(pattern="السورس")
async def zed_command_source(event):
    if event.fwd_from: return
    bot_username = Config.TG_BOT_USERNAME
    try:
        results = await zedub.tgbot.inline_query(bot_username, "السورس")
        await results[0].click(event.chat_id)
        await event.delete()
    except: pass
...
# --- أوامر الشروحات (تعتمد على الانلاين) ---
async def send_inline_help(event, query_text):
...
async def cmd_manual(event):
    await send_inline_help(event, "اليدوي")

@zedub.zed_cmd(pattern="الفيزا(?: |$)(.*)")
async def cmd_visa(event):
    await send_inline_help(event, "الفيزا")

@zedub.zed_cmd(pattern="الهيروكو(?: |$)(.*)")
async def cmd_heroku(event):
    await send_inline_help(event, "الهيروكو")

@zedub.zed_cmd(pattern="التنصيب(?: |$)(.*)")
async def cmd_install(event):
    await send_inline_help(event, "التنصيب")


# --- الأوامر النصية العادية (Fallback Text Commands) ---
# هذه الأوامر تعمل دائماً كنص عادي في حال فشل الانلاين
@zedub.zed_cmd(pattern=f"الأوامر(?: |$)(.*)")
@zedub.zed_cmd(pattern=f"اوامري(?: |$)(.*)")
async def zed_text_orders(event):
...
# تعريف الدوال النصية من م1 إلى م24
# نستخدم القاموس لتقليل التكرار
async def send_cmd_text(event, key):
    if key in CMD_TEXTS:
        await edit_or_reply(event, CMD_TEXTS[key])

@zedub.zed_cmd(pattern="م1(?: |$)(.*)")
async def cmd_m1(event): await send_cmd_text(event, "1")

@zedub.zed_cmd(pattern="م2(?: |$)(.*)")
async def cmd_m2(event): await send_cmd_text(event, "2")

@zedub.zed_cmd(pattern="م3(?: |$)(.*)")
async def cmd_m3(event): await send_cmd_text(event, "3")

@zedub.zed_cmd(pattern="م4(?: |$)(.*)")
async def cmd_m4(event): await send_cmd_text(event, "4")

@zedub.zed_cmd(pattern="م5(?: |$)(.*)")
async def cmd_m5(event): await send_cmd_text(event, "5")

@zedub.zed_cmd(pattern="م6(?: |$)(.*)")
async def cmd_m6(event): await send_cmd_text(event, "6")

@zedub.zed_cmd(pattern="م7(?: |$)(.*)")
async def cmd_m7(event): await send_cmd_text(event, "7")

@zedub.zed_cmd(pattern="م8(?: |$)(.*)")
async def cmd_m8(event): await send_cmd_text(event, "8")

@zedub.zed_cmd(pattern="م9(?: |$)(.*)")
async def cmd_m9(event): await send_cmd_text(event, "9")

@zedub.zed_cmd(pattern="م10(?: |$)(.*)")
async def cmd_m10(event): await send_cmd_text(event, "10")

@zedub.zed_cmd(pattern="م11(?: |$)(.*)")
async def cmd_m11(event): await send_cmd_text(event, "11")

@zedub.zed_cmd(pattern="م12(?: |$)(.*)")
async def cmd_m12(event): await send_cmd_text(event, "12")

@zedub.zed_cmd(pattern="م13(?: |$)(.*)")
async def cmd_m13(event): await send_cmd_text(event, "13")

@zedub.zed_cmd(pattern="م14(?: |$)(.*)")
async def cmd_m14(event): await send_cmd_text(event, "14")

@zedub.zed_cmd(pattern="م15(?: |$)(.*)")
async def cmd_m15(event): await send_cmd_text(event, "15")

@zedub.zed_cmd(pattern="م16(?: |$)(.*)")
async def cmd_m16(event): await send_cmd_text(event, "16")

@zedub.zed_cmd(pattern="م17(?: |$)(.*)")
async def cmd_m17(event): await send_cmd_text(event, "17")

@zedub.zed_cmd(pattern="م18(?: |$)(.*)")
async def cmd_m18(event): await send_cmd_text(event, "18")

@zedub.zed_cmd(pattern="م19(?: |$)(.*)")
async def cmd_m19(event): await send_cmd_text(event, "19")

@zedub.zed_cmd(pattern="م20(?: |$)(.*)")
async def cmd_m20(event): await send_cmd_text(event, "20")

@zedub.zed_cmd(pattern="م21(?: |$)(.*)")
async def cmd_m21(event): await send_cmd_text(event, "21")

@zedub.zed_cmd(pattern="م22(?: |$)(.*)")
async def cmd_m22(event): await send_cmd_text(event, "22")

@zedub.zed_cmd(pattern="م23(?: |$)(.*)")
async def cmd_m23(event): await send_cmd_text(event, "23")

@zedub.zed_cmd(pattern="م24(?: |$)(.*)")
async def cmd_m24(event): await send_cmd_text(event, "24")
