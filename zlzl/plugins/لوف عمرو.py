...
try:
    ...
    def is_locked(chat_id, type):
        return False

    def update_lock(chat_id, type, val):
        pass
...
async def wspr(event):
    ...
    try:
        ...
        if results:
            ...
            await edit_delete(
                event, "**- عذراً، لم أستطع توليد الهمسة. تأكد من النص.**", 5
            )
...
async def lock_cmd(event):
    ...
    if input_str == "الكل":
        ...
        return await edit_or_reply(
            event, "**⎉╎عـذࢪاً عـزيـزي .. لايمكنك قفـل اي شي هنـا ...𓆰**"
        )
...
async def unlock_cmd(event):
    ...
    if input_str == "الكل":
        ...
        return await edit_or_reply(
            event, "**⎉╎عـذࢪاً عـزيـزي .. لايمكنك اعـادة فتـح اي شي هنـا ...𓆰**"
        )
...
async def check_incoming_messages(event):
    ...
    if is_locked(zed_id, "button") and "@" in hhh:
        try:
            await event.delete()
            await event.reply(
                f"[ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝗭𝗧𝗛𝗢𝗡 - حمـاية المجموعـة ](t.me/ZedThon)\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n\n⌔╎**عـذࢪاً** [{user.first_name}](tg://user?id={user.id})  \n⌔╎**يُمنـع ارسـال المعـرفـات هنـا ⚠️•**",
                link_preview=False,
            )
        except Exception:
    ...
    if is_locked(zed_id, "egame") and any(char in hhh for char in persian_chars):
        try:
            await event.delete()
            await event.reply(
                f"[ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝗭𝗧𝗛𝗢𝗡 - حمـاية المجموعـة ](t.me/ZedThon)\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n\n⌔╎**عـذࢪاً** [{user.first_name}](tg://user?id={user.id})  \n⌔╎**يُمنـع التحـدث بالفارسيـه هنـا ⚠️•**",
                link_preview=False,
            )
        except Exception:
    ...
    if is_locked(zed_id, "inline") and event.message.via_bot:
        try:
            await event.delete()
            await event.reply(
                f"[ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝗭𝗧𝗛𝗢𝗡 - حمـاية المجموعـة ](t.me/ZedThon)\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n\n⌔╎**عـذࢪاً** [{user.first_name}](tg://user?id={user.id})  \n⌔╎**يُمنـع استخـدام الانلايـن في هذه المجموعـة ⚠️•**",
                link_preview=False,
            )
        except Exception:
...
async def on_user_add(event):
    ...
    if is_locked(event.chat_id, "contact") and event.user_added:
        ...
        if await is_admin(event, added_by):
            return
        ...
        for user_id in event.action_message.action.users:
            ...
            if user_obj.bot:
                try:
                    ...
                    await event.reply(
                        f"**يُمنـع اضـافة البـوتـات لـ هـذه المجمـوعـة 🚫**"
                    )
                except:
                    pass
    ...
    if event.user_joined:
        ...
        if is_locked(event.chat_id, "egame") and any(
            char in (user.first_name or "") for char in persian_names
        ):
            if await is_admin(event, user.id):
                return
            try:
...
async def kick_bots_cmd(zed):
    ...
    if con != "طرد":
        ...
        if del_u > 0:
            ...
            msg = (
                "**⎉╎مجمـوعتك/قناتـك في أمـان ✅.. لاتوجـد بوتـات في هذه المجمـوعـة ༗**"
            )
    ...
    try:
        if not (await is_admin(zed, zed.sender_id)):
            return await edit_delete(
                zed, "**⎉╎عـذࢪاً .. احتـاج الى صلاحيـات المشـرف هنـا**", 5
            )
    except:
        pass
