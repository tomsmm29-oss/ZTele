...
try:
    ...
    class MockGban:
        def is_gbanned(self, id):
            return False

        def zedgban(self, id, r):
            pass

        def catungban(self, id):
            pass

        def get_all_gbanned(self):
            return []
...
async def get_admin_channels(client):
    ...
    async for dialog in client.iter_dialogs():
        ...
        if dialog.admin_rights:
            dialog.admin_rights.delete_messages
...
async def zedgban(event):
    ...
    if sandy == 0:
        return await edit_delete(
            zede,
            "**⎉╎عــذراً .. يجـب ان تكــون مشـرفـاً فـي مجموعـة واحـده ع الأقــل **",
        )
    ...
    for dialog in san:
        ...
        try:
            ...
            await event.client(
                EditBannedRequest(dialog.input_entity, user.id, BANNED_RIGHTS)
            )
            done = True
            count += 1
...
async def zedungban(event):
    ...
    for dialog in san:
        try:
            await event.client(
                EditBannedRequest(dialog.input_entity, user.id, UNBAN_RIGHTS)
            )
...
async def gablist(event):
    ...
    if len(gbanned_users) > 0:
        for a_user in gbanned_users:
            reason_txt = (
                a_user.reason
                if hasattr(a_user, "reason") and a_user.reason
                else "لا يـوجـد"
            )
