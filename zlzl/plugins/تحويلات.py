...
try:
    ...
    from ..helpers.utils import _zedtools, _zedutils, reply_id
...
async def to_gif_cmd(event):
    ...
    try:
        ...
        catgif = await make_gif(
            event, catfile, quality, fps
        )  # يفترض وجود دالة make_gif
...
async def pic_to_gif_cmd(event):
    ...
    try:
        ...
        outframes[0].save(
            output, save_all=True, append_images=outframes[1:], duration=0.7
        )
        output.seek(0)
