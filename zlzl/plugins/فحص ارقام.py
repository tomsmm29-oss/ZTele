            ...
            # --- الحالة 2: إضافة عبر رقم الهاتف ---
                ...
                try:
                    ...
                    await new_client.sign_in(
                        phone, raw_code, phone_code_hash=phone_code_hash
                    )
                except SessionPasswordNeededError:
                    # معالجة التحقق بخطوتين
                    await conv.send_message(
                        "**🔐 الحساب محمي بكلمة سر (2FA)، أرسلها الآن:**"
                    )
                ...
                await conv.send_message(
                    f"**✅ تم تسجيل الدخول وحفظ الحساب!**\n**👤 الاسم:** {me.first_name}\n**📱 الرقم:** `{me.phone}`"
                )
                await new_client.disconnect()

            else:
                await conv.send_message(
                    "**❌ يرجى إرسال رقم هاتف يبدأ بـ (+) أو سيشن صحيح.**"
                )
...
async def save_new_acc(session, me):
    ...
    data[new_idx] = {"session": session, "name": me.first_name, "phone": me.phone}
...
async def mass_check(event):
    ...
    if not reply or not reply.text:
        return await edit_or_reply(event, "**⎉╎رد على قائمة أرقام.**")

    phones = list(dict.fromkeys(re.findall(r"\+\d{7,15}", reply.text)))
    if not phones:
        return await edit_or_reply(event, "**⎉╎لا توجد أرقام.**")
        ...
        try:
            res = await zedub(
                ImportContactsRequest(
                    [
                        InputPhoneContact(
                            client_id=0, phone=ph, first_name="Z", last_name="C"
                        )
                    ]
                )
            )
            if res.users:
                ...
                CHECK_RESULTS[ph] = {
                    "id": u.id,
                    "name": u.first_name,
                    "prem": getattr(u, "premium", False),
                }
                if u.id not in IMPORTED_IDS:
...
async def show_all(event):
    if not CHECK_RESULTS:
        return await edit_or_reply(event, "**❌ لا توجد نتائج.**")
...
async def clear_contacts_pro(event):
    ...
    if not IMPORTED_IDS:
        return await edit_or_reply(event, "**❌ القائمة فارغة.**")
